...
def _stylize_broken_emoji(text: str) -> str:
    """Thay ':ten:' (emoji bị Discord cắt mất icon) bằng **Tên** in đậm."""
    if not text:
        return text

    def _sub(m: re.Match) -> str:
        prefix = " " if m.start() > 0 and text[m.start() - 1] not in (" ", "\n") else ""
        return f"{prefix}**{m.group(1).capitalize()}**"

    return BROKEN_EMOJI_PATTERN.sub(_sub, text)
...
def _protect_tokens(text: str) -> tuple[str, dict[str, str]]:
    ...
    text = _replace(CUSTOM_EMOJI_PATTERN, text)
...
async def process_text(text: str) -> tuple[str, bool]:
    """Xử lý 1 đoạn text: dịch Anh->Việt nếu hợp lệ + làm đẹp emoji bị vỡ,
    LUÔN bảo vệ emoji tùy chỉnh thật (<:ten:id>) và link TRƯỚC khi chạm vào
    text, để không bao giờ bị nhầm phần ":ten:" bên trong emoji thật thành
    emoji vỡ (đây là lỗi khiến icon gốc bị thay bằng **Tên** in đậm).
    ...
    # 1) Bảo vệ emoji tùy chỉnh thật (<:ten:id>) + link trước tiên
    protected_text, placeholders = _protect_tokens(text)

    # 2) Làm đẹp emoji bị vỡ (":ten:" thô, do Discord tự cắt mất icon) NGAY
    #    TỪ ĐÂY, TRƯỚC khi dịch. Nếu để dịch trước rồi mới stylize, Google
    #    Translate có thể chèn khoảng trắng/đổi hoa-thường quanh dấu ":",
    #    khiến regex nhận diện emoji vỡ không còn khớp được nữa -> chữ thô
    #    ":ten:" lọt qua, không được in đậm.
    ...
    # 4) Khôi phục lại emoji thật/link nguyên vẹn
