...
async def process_text(text: str) -> tuple[str, bool]:
    ...
    if is_translatable_candidate(text):
        try:
            translated = await _translate_raw(protected_text)
            if translated.strip().lower() != protected_text.strip().lower():
                working = translated
        except Exception as e:
            log.warning(f"Dịch lỗi, giữ nguyên đoạn gốc: {e}")

    # Chạy trên bản đã được bảo vệ -> không thể đụng vào emoji thật/link nữa
    working = _stylize_broken_emoji(working)
