...
async def translate_maybe(text: str) -> str | None:
    ...
    translated = _restore_tokens(translated, placeholders)
    return translated
...
async def translate_embed(embed: discord.Embed) -> tuple[discord.Embed, bool]:
    ...
    async def tr(value):
        ...
        new_value = result if result is not None else value
        new_value = _stylize_broken_emoji(new_value)
        if new_value != value:
            changed = True
        return new_value
...
async def on_message(message: discord.Message):
    ...
    stylized_content = _stylize_broken_emoji(final_content) if final_content else final_content
    content_changed = (translated_content is not None) or (stylized_content != message.content)

    if not content_changed and not embeds_changed:
        return  # không có gì để dịch/sửa -> giữ nguyên tin gốc, không đụng vào

    final_content = stylized_content
