...
GLOSSARY: list[tuple[re.Pattern, str]] = [
    ...
    (re.compile(r"\brolled\b", re.IGNORECASE), "đã roll"),
    (re.compile(r"\brolling\b", re.IGNORECASE), "đang roll"),
    (re.compile(r"\bclaims?\b", re.IGNORECASE), "bắt"),
    (re.compile(r"\bclaimed\b", re.IGNORECASE), "đã bắt"),
...
def _protect_tokens(text: str) -> tuple[str, dict[str, str]]:
    ...
    def _replace(pattern: re.Pattern, s: str, override: str | None = None) -> str:
        ...
        def _sub(m: re.Match) -> str:
            nonlocal counter
            key = chr(0xE000 + counter)
            # override != None nghĩa là đây là từ trong GLOSSARY: khi khôi phục
            # sẽ trả về ĐÚNG NGHĨA TIẾNG VIỆT quy định sẵn, KHÔNG phải giữ
            # nguyên bản gốc (khác với $lệnh/link vốn phải giữ y hệt).
    ...
    # Sau đó mới tới từ điển thuật ngữ riêng — làm sau $lệnh/link để không bao
    # giờ khớp nhầm vào bên trong 1 tên lệnh (vd. "$rollcall" đã bị $lệnh nuốt
    # trọn thành 1 token từ bước trên rồi, glossary sẽ không đụng vào nữa).
    for pattern, vi_term in GLOSSARY:
        text = _replace(pattern, text, override=vi_term)
